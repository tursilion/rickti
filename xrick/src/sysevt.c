...
static int processEvent()
{
    ...
    if (KSCAN_KEY == JOY_FIRE) {
        SETBIT(control_status, CONTROL_FIRE);    // just one check, no need to cache
    ...
    else if (key == JOY_RIGHT) SETBIT(control_status, CONTROL_RIGHT);

    key = KSCAN_JOYY;
    if (key == JOY_UP) SETBIT(control_status, CONTROL_UP);
    else if (key == JOY_DOWN) SETBIT(control_status, CONTROL_DOWN);
#endif
